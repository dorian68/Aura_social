      ...
      <header className="border-b border-[#1e2820] px-4 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
          ...
          <a
            ...
            className="text-xs px-3 py-1.5 rounded-lg border border-[#1e2820] text-white/50 hover:text-white/80 hover:border-white/20 transition-colors shrink-0"
          >
            View club ↗
          </a>
        </div>
      ...
      {showForm && (
        <div className="bg-[#0B0F0E] border border-[#1e2820] rounded-xl p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input type="number" min="1" placeholder="Points reward*" value={form.pointsReward} onChange={e => setForm(f => ({ ...f, pointsReward: e.target.value }))} />
            <select value={form.verificationMethod} onChange={e => setForm(f => ({ ...f, verificationMethod: e.target.value }))}>
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="bg-[#0B0F0E] border border-[#1e2820] rounded-xl p-5 space-y-3">
          ...
          <input placeholder="Description (optional)" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
          <div className="grid grid-cols-2 gap-3">
            <input type="number" min="1" placeholder="Points cost*" value={form.pointsCost} onChange={e => setForm(f => ({ ...f, pointsCost: e.target.value }))} />
            <input type="number" min="1" placeholder="Stock limit (optional)" value={form.stock} onChange={e => setForm(f => ({ ...f, stock: e.target.value }))} />
          </div>
          <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}>
...
const PLATFORM_META: Record<string, { icon: string; label: string; color: string }> = {
  ...
  twitch:    { icon: "🟣", label: "Twitch",    color: "#9147FF" },
    ...
    <div className="space-y-6">
      ...
      {accounts.length > 0 && (
        <div className="space-y-2">
          ...
          {accounts.map(a => {
            const meta = PLATFORM_META[a.platform] ?? { icon: "🔗", label: a.platform, color: "#888" };
            return (
      ...
      <div className="space-y-2">
        ...
        {configured.length === 0 && (
          <p className="text-sm text-white/30 py-4 text-center">No OAuth providers configured on this server. Set the required environment variables to enable platform connections.</p>
        )}
        {configured.filter(p => !connectedSet.has(p)).map(p => {
          const meta = PLATFORM_META[p] ?? { icon: "🔗", label: p, color: "#888" };
          return (
            <div key={p} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[#0B0F0E] border border-[#1e2820]">
              ...
              <a
                href={`/api/auth/platforms/${p}/start?creatorId=${community.creatorId}&redirectAfter=${encodeURIComponent(`/dashboard/${community.id}`)}`}
                className="text-xs px-3 py-1.5 rounded-lg font-semibold text-[#060A08] shrink-0"
